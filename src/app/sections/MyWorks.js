...
import pro from "../assets/chat-collect.gif";
    ...
    <div id="my-works" className="min-h-screen py-10 max-w-screen-lg mx-auto">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 mt-10 px-4">
        {myRecentWorksData.map((item) => (
          <div
            key={item}
            className="relative cursor-pointer rounded-3xl overflow-hidden bg-landing-noise dark:bg-dark-noise"
          >
            <Image className="w-full" src={pro} />
            <div className="p-3">
              <h3 className="text-lg font-bold">{item.title}</h3>
              <h5 className="text-sm font-semibold text-primary-orange">{item.subtitle}</h5>
              <p className="mt-3 text-sm">{item.desc}</p>
              <div className="mt-2 flex flex-wrap gap-1">
                {item.tech.map((tech, idx) => (
                  <div
                    key={idx}
                    className="inline-flex items-center font-semibold rounded-md bg-black opacity-70 text-white p-1 px-2 text-[10px]"
                  >
                    {tech}
                  </div>
                ))}
